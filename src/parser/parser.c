...
/*   Updated: 2025/06/04 20:21:49 by mavissar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 			tmp = tmp->next;
// 			if (!tmp || tmp->type != WORD)
// 				return (parse_error(list, current,
// 						"expected file after redirection"));
...
static int	handle_token(t_token **tmp, t_command **current, t_command **list)
{
	if ((*tmp)->type == WORD)
		add_arg(&(*current)->args, (*tmp)->value);
	else if ((*tmp)->type == RED_IN)
	...
	else if ((*tmp)->type == RED_OUT || (*tmp)->type == APPEND)
	{
		...
		*tmp = (*tmp)->next;
		if (!(*tmp) || (*tmp)->type != WORD)
			return (parse_error(*list, *current,
					"expected file after redirection") != NULL);
		(*current)->outfile = ft_strdup((*tmp)->value);
	}
	else if ((*tmp)->type == PIPE)
	{
...
t_command	*parser(t_token *token)
{
	...
	while (tmp)
	{
		if (handle_token(&tmp, &current, &list))
			return (NULL);
		tmp = tmp->next;
	}
	add_cmd(&list, current);
