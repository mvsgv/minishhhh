...
/*   Updated: 2025/06/04 20:46:40 by mavissar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*env_var_expansion(const char *str, int *i, t_env *env)
{
	...
	tmp = ft_strdup(val);
	// Duplicate the value (do not include anything before the variable)
	free(key);
	if (!tmp)
...
static int	handle_dollar(const char *str, int *i, char **res, t_env *env,
		int exit_cd)
{
	char	*tmp;
	char	*new_res;
	...
	{
		tmp = env_var_expansion(str, i, env);
		if (!tmp)
		{
			free(*res);
			return (1);
...
char	*expand_word(const char *str, t_env *env, int exit_cd)
{
	...
	while (str[i])
	{
		...
		if (str[i] == '$' && !in_sq)
		{
			if (handle_dollar(str, &i, &res, env, exit_cd))
				return (NULL);
		}
		else
		{
